...
?>
	...
	<body class="login no-js <?php echo esc_attr(implode(' ', $classes)); ?>">
		...
		<div class="global_box">
			<div class="left_box">
				<div class="logo">
					<img src="<?php echo site_url('/wp-admin/images/Sparing_logo_all.png'); ?>" alt="Sparingconsulting Logo" class="sc_logo">
				</div>
				<div class="welcome"> <span class="sc_text"> Welcome to the <b class="sc_red">Sparing Consulting Dashboard</b></span>
				</div>
			</div>
 ...
 */
	...
	?>
		...
		<form class="admin-email-confirm-form" name="admin-email-confirm-form" action="<?php echo esc_url(site_url('signin.php?action=confirm_admin_email', 'login_post')); ?>" method="post">
			<?php
						/**
					...
					case 'retrievepassword':
						if ($http_post) {
							...
							if (!is_wp_error($errors)) {
								$redirect_to = !empty($_REQUEST['redirect_to']) ? $_REQUEST['redirect_to'] : 'signin.php?checkemail=confirm';
								wp_safe_redirect($redirect_to);
								exit;
							}
						}
	...
	?>
		...
		<form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url(network_site_url('signin.php?action=lostpassword', 'login_post')); ?>" method="post">
			<p>
					...
					case 'rp':
						...
						if (!$user || is_wp_error($user)) {
							...
							if ($user && $user->get_error_code() === 'expired_key') {
								wp_redirect(site_url('signin.php?action=lostpassword&error=expiredkey'));
							} else {
								wp_redirect(site_url('signin.php?action=lostpassword&error=invalidkey'));
							}
	...
	?>
		<form name="resetpassform" id="resetpassform" action="<?php echo esc_url(network_site_url('signin.php?action=resetpass', 'login_post')); ?>" method="post" autocomplete="off">
	...
	<?php
					...
					case 'register':
						...
						if (!get_option('users_can_register')) {
							wp_redirect(site_url('signin.php?registration=disabled'));
							exit;
						}
						...
						if ($http_post) {
							...
							if (!is_wp_error($errors)) {
								$redirect_to = !empty($_POST['redirect_to']) ? $_POST['redirect_to'] : 'signin.php?checkemail=registered';
								wp_safe_redirect($redirect_to);
								exit;
							}
						}
	...
	?>
		<form name="registerform" id="registerform" action="<?php echo esc_url(site_url('signin.php?action=register', 'login_post')); ?>" method="post" novalidate="novalidate">
			<p>
		...
		<p id="nav">
			...
			<?php
						...
						/** This filter is documented in signin.php */
		...
		<?php
					...
					case 'checkemail':
						...
						/** This action is documented in signin.php */
		...
		<form name="loginform" id="loginform" action="<?php echo esc_url(site_url('signin.php', 'login_post')); ?>" method="post">
			<p>
				<label for="user_login"><?php _e('Username or Email Address'); ?></label>
